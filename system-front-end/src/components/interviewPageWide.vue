  ...
  <div class="tool-container">
    <div class="left-tool-group">
      <div class="camera-button-open" v-if="isCameraOpen">
        <i class="bi bi-camera-video" style="font-size: 1.5em; line-height: 2.7vw;"></i>
      </div>
      <div class="camera-button-close" v-if="!isCameraOpen">
        <i class="bi bi-camera-video-off" style="font-size: 1.5em; line-height: 2.7vw;"></i>
      </div>
      <div class="mic-button-open" v-if="isMicOpen">
        <i class="bi bi-mic" style="font-size: 1.5em; line-height: 2.7vw;"></i>
      </div>
      <div class="mic-button-close" v-if="!isMicOpen">
        <i class="bi bi-mic-mute" style="font-size: 1.5em; line-height: 2.7vw;"></i>
      </div>
    </div>
    <div class="tool-text-box">
      <div class="tool-text">请您在确认音视频权限均已打开后，大声回答下方访谈的六个问题。</div>
    </div>
    <div class="media-box">
      ...
      <div class="question-util-box">
        ...
        <div class="answer-box" v-show="item.answer !== 0">  <!-- item.answer !== 0 -->
          <div class="answer-mix" disabled="item.status" @click="onClickAudio">
            <i class="bi bi-play-fill" style="margin-right: 7vw" v-show="!item.status && !this.isPlaying"></i>
            <i class="bi bi-pause-fill" style="margin-right: 7vw" v-show="!item.status && this.isPlaying"></i>
            <i class="bi bi-check" style="margin-right: 7vw" v-show="item.status"></i>
            {{item.answer}}''
...
export default {
  ...
  methods: {
    ...
    checkResult(){
      //this.$router.push('/waiting_wide');
      this.$router.push({name:'waiting-result-page-wide', query: {id: this.id}});
    }
  }
}
</script>
...
<style scoped>
@import "./stylesheets/interview-page-wide.css";
</style>